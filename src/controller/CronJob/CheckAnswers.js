  ...
  try {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        console.log(`Updated user ${user._id} wallet balance from ${previousBalance} to ${user.walletBalance} after win of ${winAmount}`);
      });
      session.endSession();
      ...
      if (updatedUser.walletBalance !== user.walletBalance) {
        console.error(`ERROR: User balance update verification failed. Expected: ${user.walletBalance}, Actual: ${updatedUser.walletBalance}`);
  ...
  try {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        console.log(`Updated user ${user._id} wallet balance from ${previousBalance} to ${user.walletBalance} after refund of ${refundAmount}`);
      });
      session.endSession();
  ...
  try {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        console.log(`Updated user ${user._id} wallet balance from ${previousBalance} to ${user.walletBalance} after partial refund of ${unmatchedAmount}`);
      });
      session.endSession();
