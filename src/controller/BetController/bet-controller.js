...
const updatePlayFabInventory = async (user, amount, transactionId, source = "bet") => {
  try {
    console.log("🎯 === UPDATE PLAYFAB INVENTORY FUNCTION CALLED (BET CONTROLLER) ===")
    console.log(`🎯 User ID: ${user._id}, Amount: ${amount}, Source: ${source}`)
    ...
    // 4. Call the centralized PlayFab service to process the transaction.
    console.log(
      `🚀 Calling playFabService.processPaymentToPlayFab for a ${amount > 0 ? "credit" : "debit"} of ${Math.abs(amount)}...`,
    )
    const result = await playFabService.processPaymentToPlayFab(user, amount, transactionId, {
      source,
...
// Process all bets for a resolved question - FIXED VERSION WITH PROPER PLAYFAB UPDATES
  ...
  try {
    ...
    for (const bet of bets) {
      ...
      if (isWinner) {
        ...
        console.log(`🏆 === BET WIN PROCESSING FOR BET ${bet._id} ===`)
        ...
        // *** PLAYFAB INTEGRATION: NO ACTION NEEDED FOR LOSS ***
        // The amount was already deducted from PlayFab when the bet was placed.
        console.log(`💸 === BET LOSS PROCESSING FOR BET ${bet._id} ===`)
        console.log(`💸 User: ${user._id}`)
        console.log(`💸 Bet ${bet._id} lost. PlayFab balance was already deducted during bet placement.`)
        console.log(`💸 No further PlayFab action needed for loss.`)
        bet.status = "lost"
...
// Add debug controller for troubleshooting
exports.debugController = async (req, res) => {
  try {
    console.log("=== DEBUG CONTROLLER CALLED ===")
    ...
    res.status(500).json({
      success: false,
      message: "Debug controller error",
      error: error.message,
    })
  }
}
