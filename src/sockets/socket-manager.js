...
const generateRandomQuestion = async () => {
  try {
    ...
    if (!Array.isArray(data) || data.length === 0 || !data[0].CameraHolderName || data[0].CameraHolderName === "None") {
      throw new Error("No valid camera holder data found")
    }

    // Use the camera holder name as the subject
    const subject = data[0].CameraHolderName
...
async function checkCameraHolderChanges() {
  try {
    ...
    // If camera holder changed from None/empty to a valid name
    if (
      (lastCameraHolderName === null || lastCameraHolderName === "" || lastCameraHolderName === "None") &&
      currentCameraHolder && 
      currentCameraHolder !== "None"
      ...
      if (!data[0].CameraHoldStartTime) {
        try {
          await fetch(`http://localhost:5000/api/players/${data[0]._id}`, {
            ...
            body: JSON.stringify({ 
              CameraHolderName: currentCameraHolder 
            }),
          });
          console.log(`Updated CameraHoldStartTime for ${currentCameraHolder}`);
        } catch (error) {
          console.error("Error updating CameraHoldStartTime:", error);
        }
      }
    ...
    // If camera holder changed from a valid name to None (player died or lost camera)
    else if (
      lastCameraHolderName && 
      lastCameraHolderName !== "None" && 
      (currentCameraHolder === "None" || !currentCameraHolder)
    ) {
      console.log(`Camera holder changed from ${lastCameraHolderName} to None`);
      ...
      try {
        await fetch(`http://localhost:5000/api/players/${data[0]._id}`, {
          ...
          body: JSON.stringify({ 
            CameraHolderName: "None" 
          }),
        });
        console.log(`Updated player stats for ${lastCameraHolderName} who lost the camera`);
      } catch (error) {
...
module.exports = async function setupSocketIO(server) {
  ...
  async function sendActiveQuestion(socket, streamId = "default-stream") {
    try {
      ...
      if (activeQuestion) {
        // Rest of the existing code for active question...
      ...
      const shouldGenerate = await shouldGenerateQuestions()
      if (!shouldGenerate) {
        console.log("Skipping fallback question - camera holder conditions not met")
        socket.emit("no_questions_available", {
          message: "No questions available at this time",
        })
        return
